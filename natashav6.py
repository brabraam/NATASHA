...
def meteo():
    city = "Lausanne"
    res = requests.get(f"http://api.openweathermap.org/data/2.5/weather?q={city}&appid=<6f607d60432bb6936eda844951d3ec7b>&units=metric").json()
...
def news():
    ...
    speak("sur quels sujets voulez vous des news")
...
def Take_query():
    ...
    while (True):
        ...
        elif "calcul toi plus moi" in query:
            speak("vous et moi c'est pour la vie")

        elif "Natacha" in query:
            speak("c'est moi")

        elif "Abraham" in query:
            speak("oh, c'est le grand chad")

        elif "quoi" in query:
            speak("feur")

        elif "mathis" in query:
            speak("tisma tobo")

        elif "ben" in query:
            speak("ho, ho, ho")

        elif "ronaldo" in query:
            speak("cristiano ronaldo sou i")

        elif "pierre" in query:
            speak("le big P?")
